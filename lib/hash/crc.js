...
function Crc8Str(str) {
    let crc = 0;
    for (const byte of Utf8Bytes(str)) crc = Crc8Add(crc, byte);
    return crc;
}

function CrcArcStr(str) {
    let crc = 0;
    for (const byte of Utf8Bytes(str)) crc = CrcArcAdd(crc, byte);
    return crc;
}

function Crc16Str(str) {
    let crc = 0;
    for (const byte of Utf8Bytes(str)) crc = Crc16Add(crc, byte);
    return crc;
}

function Fcs16Str(str) {
    let fcs = 0xFFFF;
    for (const byte of Utf8Bytes(str)) fcs = Fcs16Add(fcs, byte);
    return fcs ^ 0xFFFF;
}

function Crc32Str(str) {
    let crc = 0xFFFFFFFF;
    for (const byte of Utf8Bytes(str)) crc = Crc32Add(crc, byte);
...
function Hex8(val) {
  ...
  return "0x" + (str.length < 2 ? "0" + str : str);
}

function Hex16(val) {
  let n = val & 0xFFFF;
  let str = n.toString(16).toUpperCase();
  return "0x" + (str.length < 4 ? "0".repeat(4 - str.length) + str : str);
}


function Hex32(val) {
    let str1 = (val & 0xFFFF).toString(16).toUpperCase();
    let str2 = ((val >>> 16) & 0xFFFF).toString(16).toUpperCase();
    return "0x" + (str2.length < 4 ? "0".repeat(4 - str2.length) + str2 : str2) + (str1.length < 4 ? "0".repeat(4-str1.length) + str1 : str1);
}
